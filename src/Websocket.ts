...
export class Websocket extends EventEmitter {
	...
	async connect(token: string) {
		return new Promise<void>(async (resolve) => {
			this.ws = new WebSocket("wss://gateway.discord.gg/gateway/bot?v=10&encoding=json");
			...
			this.ws.on("message", async (rawData) => {
				...
				this.client.emit("debug", `Received event: ${event.op || "Opcode N/A"} | ${event.t || "Type N/A"}`);
	...
	async sendHeartbeat(noDebug: boolean = false) {
		return new Promise<void>((resolve) => {
			...
			this.ws.send(payload, (err) => {
				if (err) throw new WebsocketError("Cannot send heartbeat: " + err.message);
				resolve();
			});
		});
	}
